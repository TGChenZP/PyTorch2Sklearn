...
class TorchToSklearn_ImageGraphModel(object):
    ...
    def fit(self, train_x: pd.DataFrame, train_x_images: dict, train_y: pd.Series):
        """Fit the model
        ...
        x_list, x_images_list, y_list = self.CFG["ImageGraphDataFactory"](
            train_x, train_x_images, train_y, self.CFG["mode"], self.CFG
        )  # might want to do batch number as well
        ...
        if self.CFG["verbose"] == True:
            for epoch in tqdm(range(self.CFG["epochs"])):
                ...
                for mini_batch_number in range(len(x_list)):
                    X, X_images, y = torch.FloatTensor(np.array(x_list[mini_batch_number])).to(
                        self.device
                    ).squeeze(0), torch.FloatTensor(np.array(x_images_list[mini_batch_number])).to(
                        self.device
                    ).squeeze(0),
                    torch.FloatTensor(
                        np.array(y_list[mini_batch_number])
            ...
            for epoch in range(self.CFG["epochs"]):
                ...
                for mini_batch_number in range(len(x_list)):
                    ...
                    torch.FloatTensor(x_images_list[mini_batch_number]).to(
                        self.device
                    ),
                    torch.FloatTensor(
        ...
        with torch.no_grad():
            ...
            for mini_batch_number in range(len(x_list)):
                x_batch, x_images_batch = torch.FloatTensor(x_list[mini_batch_number]).to(self.device).squeeze(0), \
                    torch.FloatTensor(
                        x_images_list[mini_batch_number]
        ...
        with torch.no_grad():
            ...
            for mini_batch_number in range(len(x_list)):
                x_batch, x_images_batch = torch.FloatTensor(
                    x_list[mini_batch_number]).to(self.device).squeeze(0), torch.FloatTensor(x_images_list[mini_batch_number]).to(self.device).squeeze(0)
